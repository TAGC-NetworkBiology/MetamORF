...
# AddReleaseVersion
echo "AddReleaseVersion strategy" >> ${README_FILE_PATH}
echo "- Started on $(date "+%d/%m/%y, at %H:%M:%S")" >> ${README_FILE_PATH}
echo "  python $PYTHONPATH/fr/tagc/uorf/uorf.py AddReleaseVersion -v $VERBOSITY_LEVEL -T $DB_TYPE -H $DB_HOST -P $DB_PORT -u $DB_USER -p $DB_PASSWORD -N $DS_DB_NAME -M DS -r 1.0 -d $DS_DB_NAME" >> ${README_FILE_PATH}
echo "" >> ${README_FILE_PATH}
python $PYTHONPATH/fr/tagc/uorf/uorf.py AddReleaseVersion -v $VERBOSITY_LEVEL -T $DB_TYPE \
  -H $DB_HOST -P $DB_PORT -u $DB_USER -p $DB_PASSWORD -N $DS_DB_NAME -M DS -r 1.0 -d $DS_DB_NAME
...
echo "AddReleaseVersion strategy" >> ${README_FILE_PATH}
echo "- Started on $(date "+%d/%m/%y, at %H:%M:%S")" >> ${README_FILE_PATH}
echo "  python $PYTHONPATH/fr/tagc/uorf/uorf.py AddReleaseVersion -v $VERBOSITY_LEVEL -T $DB_TYPE -H $DB_HOST -P $DB_PORT -u $DB_USER -p $DB_PASSWORD -N $PRO_DB_NAME -M PRO -r 1.0 -d $PRO_DATABASE_NAME" >> ${README_FILE_PATH}
echo "" >> ${README_FILE_PATH}
python $PYTHONPATH/fr/tagc/uorf/uorf.py AddReleaseVersion -v $VERBOSITY_LEVEL -T $DB_TYPE \
  -H $DB_HOST -P $DB_PORT -u $DB_USER -p $DB_PASSWORD -N $PRO_DB_NAME -M PRO -r 1.0 -d $PRO_DB_NAME
